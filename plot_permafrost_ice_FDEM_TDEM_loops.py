...
import Cole_models
from halfspace_and_3layers import halfspace_and_3layers
...
plotcolors = ['black','gray', 'silver', 'teal', 'C2','C3','C4','C5', 'C6', 'C7', 'C8', 'C9', 'grey', 'gold', 'b', 'g', 'r', 'c', 'm', 'crimson', 'pink', 'teal', 'sandybrown']
...
saveafig = 'no'
#saveafig = 'yes'
filename = 'permafrost_1m_ice_layer_at_table_1m.pdf'


saveafig_fd = 'no'
#saveafig_fd = 'yes'
filename_fd = 'permafrost_05m_ice_layer_at_table_0m_fd.pdf'
...
#depth=[0, 1, 2]  # Husk! *Nedre lag er under siste oppgitte dybde.      # Depth-model, adding air-interface.
#depth=[0, 2, 4]  # Husk! *Nedre lag er under siste oppgitte dybde.      # Depth-model, adding air-interface.
depth=[0, 10, 20]  # Husk! *Nedre lag er under siste oppgitte dybde.      # Depth-model, adding air-interface.
#depth=[0, 5, 6]  # Husk! *Nedre lag er under siste oppgitte dybde.      # Depth-model, adding air-interface.




for subsurf2 in [Fairbanks_silt_44_ice_minus_3, Fairbanks_silt_72_ice_minus_3, Fairbanks_silt_100_ice_minus_3]:

#for subsurf2 in [Tre_tusen]:

#for subsurf2 in [Fairbanks_silt_72_ice_minus_3]:
#for subsurf2 in [Fairbanks_silt_44_ice_minus_3]:
#for subsurf2 in [Tre_tusen, Fairbanks_silt_100_ice_minus_3]:
...
#	subsurf3 = subsurf2
	...
	subsurf1 = Tre_tusen # for halfspace, set subsurf2 and 3 = subsurf1
	subsurf3 = Tre_tusen
...
#	radius = np.array([1,  5, 10, 100]) # or siden length 
	radius = np.array([4.99]) # or siden length 
...
#	output_H_or_EMF = 'H'
	...
	I = 3
	Nr = 31
	a_rec = 1/((np.pi)**.5) # gives A_rec = np.pi*(a_rec**2) = 1. Videre blir receiver_moment = 31.4, slik som for SkyTEM (med Nr=31)
...
#	filter_ht = 'default'
	...
	off_time, times, out_custom, out_step_off, radius, rampdown_end, Ar, Nr, I, soln_type,f, Ippm, Qppm, Ippm_c, Qppm_c, Ippm_c_diff, Qppm_c_diff, I_fd, N_t_fd, theta, ylabeltext, noise, noiselabel = halfspace_and_3layers(depth, subsurf1, subsurf2, subsurf3, radius, I, Nr, a_rec, Tx_geometry,  Rx_config, normalization, filter_ft, filter_ht, hTx, hRx, custom_LPF, output_H_or_EMF, soln_type, h_fd, s_fd, a_fd)
...
#			ax1.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+y*np.int(len(radius))], marker=mrk[i+ikx+y], markevery=markerspace[y+i+ikx], label=r'%.45s at %.1d K, a = %.2f m' %(subsurf2.name, (subsurf2.T + 273 ), radius[i])  )
		
#			ax2.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+np.int(y*len(radius))], marker=mrk[i+ikx+y],  markevery=markerspace[y+i+ikx], label=r'%.45s at %.1d K, a = %.2f m' %(subsurf2.name, (subsurf2.T  + 273 ), radius[i])  )

			ax1.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+y*np.int(len(radius))], marker=mrk[i+ikx+y], markevery=markerspace[y+i+ikx], label=r'%.45s, a=%.1f' %(subsurf2.name, radius[i])  )
		
			ax2.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+np.int(y*len(radius))], marker=mrk[i+ikx+y],  markevery=markerspace[y+i+ikx], label=r'%.45s, a=%.1f' %(subsurf2.name, radius[i])  )
...
### FD ### FD### FD### FD### FD### FD### FD### FD### FD ### FD ### FD### FD### FD### FD### FD### FD### FD### FD
	...
	plt.figure(4)
	plt.plot(f, np.abs(-Ippm_c), '-', linewidth=2, markersize=10, color = plotcolors[ikx], marker=mrk[ikx], markevery=markerspace[ikx], label=r'Full model. I. %.45s at %.1d K, a=%.1f' %(subsurf2.name, (subsurf2.T + 273 ), radius[i]))
	plt.plot(f, np.abs(-Qppm_c), '--', linewidth=2, markersize=10, color = plotcolors[ikx], marker=mrk[ikx+2], markevery=markerspace[ikx+2], label=r'Full model. Q. %.45s at %.1d K, a=%.1f' %(subsurf2.name, (subsurf2.T + 273 ), radius[i]))

#	plt.plot(f, np.abs(-Ippm_c_diff), '-', linewidth=2,markersize=10, color = plotcolors[i+ikx+8], marker=mrk[y+4], markevery=markerspace[y+i+ikx+4], label=r'Diffusive model. I. %.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 )) )

#	plt.plot(f, np.abs(-Qppm_c_diff), '--', linewidth=2, markersize=10,color = plotcolors[i+ikx+12], marker=mrk[y+6], markevery=markerspace[y+i+ikx+6], label=r'Diffusive model. Q. %.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 )))
...
plt.rcParams['text.usetex'] = True
fig.suptitle(r'TDEM response of ice layers in permafrost', fontsize = titlefont)
...
ax1.plot(off_time, np.ones(len(off_time))*noise, ':', color='gray', linewidth=3)#, label = noiselabel)
ax2.plot(off_time, -np.ones(len(off_time))*noise, ':', color='gray', linewidth=3)
...
# Upstairs
ax1.set_yticks([ 1e-14, 1e-12, 1e-10, 1e-8, 1e-6, 1e-4, 1e-2]) # den oppe skal være rett tall
# Downstairs
ax2.set_yticks([-1e-2, -1e-4, -1e-6, -1e-8, -1e-10, -1e-12, -1e-14, -1e-16 ])
ax2.set_yticklabels([r'$-10^{-2}$', r'$-10^{-4}$', r'$-10^{-6}$',  r'$-10^{-8}$', r'$-10^{-10}$', r'$-10^{-12}$', r'$-10^{-14}$', r'$\pm10^{-16}$'  ])
plt.tight_layout()
...
if subsurf1 == subsurf2:
	...
	ax2.text(xlayer_text,-1e-5, r'Layer 1 (0 - %.1f m): %.45s'%(depth[1], Name ), 				fontsize=legendfont)

#	ax2.text(xlayer_text,-1e-4, r'Layer 2 (%.1f m - %.1f m) %.45s ' %(depth[1], depth[2], Name2),		fontsize=font)
	ax2.text(xlayer_text,-1e-3, r'Layer 2 (%.1f m - %.1f m): %.45s' %(depth[1], depth[2], 'Ice/silt'),		fontsize=legendfont)

#	ax2.text(xlayer_text,-1e-3, r'Layer 2 (%.2f m - %.1f m): %.45s or %.45s' %(depth[1], depth[2], MUT[0], MUT[1]),		fontsize=legendfont)
	ax2.text(xlayer_text,-1e-1, r'Layer 3 (%.1f m - $\infty$): %.45s ' %(depth[2], Name3 ),	 fontsize=legendfont)
...
plt.figure(3)
plt.title(r'Empymod - repr. Fig. 4 of Huang et al. 2005')
plt.plot(theta, -Ippm, '-',  label=r'I. a = %.2f m' %(a_fd) )
plt.plot(theta, -Qppm, '--', label=r'Q. a = %.2f m' %(a_fd) )
plt.xscale('log')
plt.yscale('log')
plt.xlim([0.001,1])
plt.ylim([0.1,10000])
plt.xlabel(r'$\theta$', fontsize = 14)
plt.ylabel(r'$\frac{H^{s}}{H^{p}}$ (ppm)', rotation='horizontal', fontsize = 14) 
plt.legend(frameon = False)
#plt.savefig('empymod-fdem-verify-Huang-2005.pdf', format='pdf') # Must come before plt.show()
...
plt.rcParams['text.usetex'] = True
plt.figure(4)
plt.title(r'FDEM Three vertically displaced loops configuration')
plt.xscale('log')
plt.yscale('log')
plt.ylim([1e-06, 1e02])
plt.xlim([1e02, 3e05])
plt.xlabel(r'Frequency (Hz)', fontsize = font)
plt.ylabel(r'$\frac{H^{s}}{H^{p}}$ (ppm)', rotation='horizontal', fontsize = font) 
plt.legend(frameon = False)
xlayer_text_fd = 1e03
plt.tight_layout()
if subsurf1 == subsurf2:
	plt.text(xlayer_text_fd, 1e-3, r'Halfspace',	 fontsize=font)
else:
#	plt.text(xlayer_text_fd,1e01, r'Layer 1 (0 - %.1f m) %.45s'%(depth[1], Name ), 				fontsize=font)
#	plt.text(xlayer_text_fd,1e-1, r'Layer 2 (%.1f m - %.1f m) %.45s ' %(depth[1], depth[2], Name2),		fontsize=font)
#	plt.text(xlayer_text_fd,1e-3, r'Layer 3 (%.1f m - $\infty$) %.45s ' %(depth[2], Name3 ),	 fontsize=font)

	plt.text(xlayer_text_fd,1e-3, r'Layer 1 (0 - %.2f m) %.45s'%(depth[1], Name ), 	fontsize=legendfont)

	
	plt.text(xlayer_text_fd,1e-4, r'Layer 2 (%.2f m - %.1f m) %.45s' %(depth[1], depth[2], 'MUT'),	fontsize=legendfont)

#	plt.text(xlayer_text_fd,1e-6, r'Layer 2 (%.2f m - %.1f m) %.45s or %.45s' %(depth[1], depth[2], MUT[0], MUT[1]),		fontsize=font)

	plt.text(xlayer_text_fd,1e-5, r'Layer 3 (%.1f m - $\infty$) %.45s ' %(depth[2], Name3 ), fontsize=legendfont)

if saveafig_fd == 'yes':
	plt.savefig(filename_fd, format='pdf') # Must come before plt.show()
else:
	pass
