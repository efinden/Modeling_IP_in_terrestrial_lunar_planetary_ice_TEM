...
plotcolors = ['black','gray', 'silver', 'teal', 'sandybrown', 'C2','C3','C4','C5', 'C6', 'C7', 'C8', 'C9', 'grey', 'gold', 'b', 'g', 'r', 'c', 'm', 'crimson', 'pink', 'teal', 'sandybrown']
...
#saveafig = 'no'
saveafig = 'yes'
filename = 'martian_1m_ice_layer_at_table_1m.pdf'
#filename = 'martian_10m_ice_layer_at_table_1m.pdf'
#filename = 'martian_1m_ice_layer_at_table_10m.pdf'
#filename = 'martian_10m_ice_layer_at_table_10m.pdf'
...
saveafig_fd = 'no'
#saveafig_fd = 'yes'
filename_fd = 'martian_1m_ice_layer_at_table_1m_fd.pdf'


depth=[0, 1, 2]  # Husk! *Nedre lag er under siste oppgitte dybde. Fig. 6
#depth=[0, 1, 11]  # Husk! *Nedre lag er under siste oppgitte dybde. Fig. 7
#depth=[0, 10, 11]  # Husk! *Nedre lag er under siste oppgitte dybde. Fig. 8
#depth=[0, 10, 20]  # Husk! *Nedre lag er under siste oppgitte dybde. Fig. 9



# 500 m layer NOT SHOWN IN PAPER
#depth=[0, 1, 501]  # Husk! *Nedre lag er under siste oppgitte dybde. Fig. 9
#depth=[0, 10, 510]  # Husk! *Nedre lag er under siste oppgitte dybde. Fig. 9
...
#for subsurf2 in [JSC_Mars_1, Ice_1mM_CaCl2_210K, JSC_Mars_1_60_perc_ice_0_1M_CaCl2_210K]: #, Ice_0_1M_CaCl2_210K, Ice_1mM_CaCl2_210K, JSC_Mars_1_60_perc_ice_0_1M_CaCl2_210K, JSC_Mars_1_60_perc_ice_0_1M_CaCl2_198K]:


for subsurf2 in [JSC_Mars_1, Ice_0_1M_NaCl_210K, Ice_1mM_NaCl_210K, Ice_0_1M_NaCl_198K, JSC_Mars_1_60_perc_ice_0_1M_CaCl2_210K, JSC_Mars_1_60_perc_ice_0_1M_CaCl2_198K]:
...
#	subsurf3 = subsurf2
	...
	subsurf1 = JSC_Mars_1 # for halfspace, set subsurf2 and 3 = subsurf1
	subsurf3 = JSC_Mars_1
...
#		ax2.plot(off_time,  out_custom[i,:], '-', linewidth=2, color = plotcolors[i+ikx], linestyle=lines[i+ikx], label=r'Ramp-off. %.45s at %.1d K, a = %.2f m' %(subsurf.name, (subsurf.T  + 273 ) , radius[i]) )
			ax1.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+y*np.int(len(radius))], marker=mrk[i+ikx+y], markevery=markerspace[y+i+ikx], label=r'%.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 ))  )
		
			ax2.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+np.int(y*len(radius))], marker=mrk[i+ikx+y],  markevery=markerspace[y+i+ikx], label=r'%.45s at %.1d K' %(subsurf2.name, (subsurf2.T  + 273 ))  )
...
### FD ### FD### FD### FD### FD### FD### FD### FD### FD ### FD ### FD### FD### FD### FD### FD### FD### FD### FD
	...
	plt.figure(4)
	plt.plot(f, np.abs(-Ippm_c), '-', linewidth=2, markersize=10, color = plotcolors[ikx], marker=mrk[y], markevery=markerspace[y+i+ikx], label=r'Full model. I. %.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 )))
	plt.plot(f, np.abs(-Qppm_c), '--', linewidth=2, markersize=10, color = plotcolors[ikx+4], marker=mrk[y+2], markevery=markerspace[y+i+ikx+2], label=r'Full model. Q. %.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 )))
...
plt.rcParams['text.usetex'] = True
fig.suptitle(r'TDEM response of Martian ice layers', fontsize = titlefont)
...
ax1.set_ylabel('y label left', labelpad=30)
...
# Upstairs
ax1.set_yticks([1e-16, 1e-14, 1e-12, 1e-10, 1e-8, 1e-6, 1e-4]) # den oppe skal være rett tall
# Downstairs
ax2.set_yticks([-1e-4, -1e-6, -1e-8, -1e-10, -1e-12, -1e-14, -1e-16,  -1e-18 ])
ax2.set_yticklabels([ r'$-10^{-4}$', r'$-10^{-6}$',  r'$-10^{-8}$', r'$-10^{-10}$', r'$-10^{-12}$', r'$-10^{-14}$', r'$-10^{-16}$', r'$\pm10^{-18}$'  ])
#plt.tight_layout()
...
if subsurf1 == subsurf2:
	...
	ax2.text(xlayer_text,-1e-8, r'Layer 1 (0 - %.1f m): %.45s'%(depth[1], Name ), 				fontsize=legendfont)

#	ax2.text(xlayer_text,-1e-4, r'Layer 2 (%.1f m - %.1f m) %.45s ' %(depth[1], depth[2], Name2),		fontsize=font)
	ax2.text(xlayer_text,-1e-6, r'Layer 2 (%.1f m - %.1f m): %.45s' %(depth[1], depth[2], 'MUT'),		fontsize=legendfont)

#	ax2.text(xlayer_text,-1e-3, r'Layer 2 (%.2f m - %.1f m): %.45s or %.45s' %(depth[1], depth[2], MUT[0], MUT[1]),		fontsize=legendfont)
	ax2.text(xlayer_text,-1e-4, r'Layer 3 (%.1f m - $\infty$): %.45s ' %(depth[2], Name3 ),	 fontsize=legendfont)
...
plt.ylabel(r'$\frac{H^{s}}{H^{p}}$ (ppm)',  fontsize = font) # rotation='horizontal', 
plt.legend(frameon = False)
xlayer_text_fd = 3e01
#plt.tight_layout()
if subsurf1 == subsurf2:
