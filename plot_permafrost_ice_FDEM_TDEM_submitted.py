...
depth=[0, 7, 8]  # Husk! *Nedre lag er under siste oppgitte dybde.      # Depth-model, adding air-interface.




for subsurf2 in [Tre_tusen,Fairbanks_silt_44_ice_minus_3, Fairbanks_silt_72_ice_minus_3, Fairbanks_silt_100_ice_minus_3]:
...
#			ax2.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+np.int(y*len(radius))], marker=mrk[i+ikx+y],  markevery=markerspace[y+i+ikx], label=r'%.45s at %.1d K, a = %.2f m' %(subsurf2.name, (subsurf2.T  + 273 ), radius[i])  )
			...
			ax1.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+y*np.int(len(radius))], marker=mrk[i+ikx+y], markevery=markerspace[y+i+ikx], label=r'%.45s' %(subsurf2.name)  )
		
			ax2.plot(times, out_step_off[x, i,:], '-', linewidth=2, markersize=8, color = plotcolors[i+ikx+np.int(y*len(radius))], marker=mrk[i+ikx+y],  markevery=markerspace[y+i+ikx], label=r'%.45s' %(subsurf2.name)  )
...
### FD ### FD### FD### FD### FD### FD### FD### FD### FD ### FD ### FD### FD### FD### FD### FD### FD### FD### FD
	...
	plt.figure(4)
	plt.plot(f, np.abs(-Ippm_c), '-', linewidth=2, markersize=10, color = plotcolors[ikx], marker=mrk[ikx], markevery=markerspace[ikx], label=r'Full model. I. %.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 )))
	plt.plot(f, np.abs(-Qppm_c), '--', linewidth=2, markersize=10, color = plotcolors[ikx], marker=mrk[ikx+2], markevery=markerspace[ikx+2], label=r'Full model. Q. %.45s at %.1d K' %(subsurf2.name, (subsurf2.T + 273 )))
